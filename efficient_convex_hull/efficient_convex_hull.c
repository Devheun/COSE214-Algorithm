...
t_line* upper_hull(t_point* points, int num_point, t_point p1, t_point pn, t_line* lines, int* num_line, int* capacity) {
	...
	//두 점을 잇는 선 자체가 upper_hull에 포함된다.
	if (num_point == 0) {
		...
		float a2 = pn.y - p1.y;
		float b2 = p1.x - pn.x;
		float c2 = p1.x * pn.y - p1.y * pn.x;
		float dist = 0;

		for (int j = 0; j < num_point; j++) {
			t_point heun = points[j];
			if (dist < distance(a2, b2, c2, heun)) {
				dist = distance(a2, b2, c2, heun);
				pmax = heun;
			}
		}

		separate_points(points,num_point,p1,pmax,s3,s4,&n3,&n4);
		lines=upper_hull(s3,n3,p1,pmax,lines,num_line,capacity);

		free(s3);
		free(s4);

		s3 = (t_point*)malloc(sizeof(t_point) * num_point);
		assert(s3 != NULL);
	    s4 = (t_point*)malloc(sizeof(t_point) * num_point);
		assert(s4 != NULL);

		separate_points(points, num_point, pmax, pn, s3, s4, &n3, &n4);
		lines=upper_hull(s3, n3, pmax, pn, lines, num_line, capacity);
...
void separate_points(t_point* points, int num_point, t_point from, t_point to, t_point* s1, t_point* s2, int* n1, int* n2) {
	...
	*n2 = 0;
	...
	for (int i = 0; i < num_point; i++) {
		t_point si = points[i];
		int somepoint;
		somepoint = a * si.x + b * si.y - c;

		if (somepoint == 0)continue; //from이나 to 자신은 패스
